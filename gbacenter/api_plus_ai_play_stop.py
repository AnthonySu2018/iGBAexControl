'''
这一个版本的api,要兼容可以接收aibox的推送，接受到推送后，就开始播放或停止。
首先要解析推送，然后发送命令。
...
server_running = True
last_command = None
countdown_duration = 150 # 倒计时持续时间（秒）

# 初始化数据存储字典
received_data = {}

# 处理数据的函数，验证数据是否已经在150秒内收到过，如果不是则执行操作
...
class HandlePadCommand:
    ...
    def process_command(self, data: dict, commands_dic:dict, visit_dic: dict, device_dic: dict) -> object:
        ...
        if 'algo_name' in data:
            #抽烟播放视频，打电话停止
            #收到第一个后，如果两分钟内收到第二个，忽略
            local_dic = {'test01':'序厅大屏', 'test02':'投影仪', 'test03':'Intel Vision',
                         '打电话识别':'视频1' , '抽烟识别':'停止'}

            camera_name = data.get('camera_name')
            algo_name = data.get('algo_name')

            if algo_name in local_dic and camera_name in local_dic:
                print(algo_name,camera_name)
                str_commands = visit_dic[local_dic[camera_name]][local_dic[algo_name]]

                current_time = time.time()
                if str_commands not in received_data or current_time - received_data[str_commands] > 150:
                    print(f"执行操作：{str_commands}")
                    print(received_data)
                    received_data[str_commands] = current_time
                else:
                    return
        else:
